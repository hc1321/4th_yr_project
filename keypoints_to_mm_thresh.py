...
def process_video(input_csv, video_id, conv_factor, threshold):
    ...
    for frame, row in df.iterrows():
        ...
        for bp in bodyparts:
            try:
                ...
                x = float(row[(scorer, 'x')]) if (scorer, 'x') in row.index and bp.lower() in row.index[0].lower() else None
            except Exception:

                try:
                    x = float(row[(bp, 'x')])
                except Exception as e:
                    print(f"Warning: Missing x for {bp} in frame {frame} of video {video_id}. Error: {e}")
                    x = float('nan')
            try:
                y = float(row[(scorer, 'y')]) if (scorer, 'y') in row.index and bp.lower() in row.index[0].lower() else None
            except Exception:
                try:
                    y = float(row[(bp, 'y')])
                except Exception as e:
                    print(f"Warning: Missing y for {bp} in frame {frame} of video {video_id}. Error: {e}")
                    y = float('nan')
            try:
                likelihood = float(row[(scorer, 'likelihood')]) if (scorer, 'likelihood') in row.index and bp.lower() in row.index[0].lower() else None
            except Exception:
                try:
                    likelihood = float(row[(bp, 'likelihood')])
                except Exception as e:
                    print(f"Warning: Missing likelihood for {bp} in frame {frame} of video {video_id}. Error: {e}")
        ...
        for i in range(n):
            for j in range(i+1, n):
                likelihood1 = kp_data[i * 3 + 2]
                likelihood2 = kp_data[j * 3 + 2]
                if (math.isnan(likelihood1) or math.isnan(likelihood2)) or (likelihood1 < threshold or likelihood2 < threshold):
                    distance_mm = 0
                else:
                    distance_mm = distance_bet_keypoints(kp_data, i, j, conv_factor)
                results.append({
                    "video_id": video_id,
                    "frame": frame,
                    "bone": f"{bodyparts[i]}-{bodyparts[j]}",
...
def main():
    ...
    base_output_folder = r"keypoints_mm2_all_lengths_threshold3"      
    ...
    for thresh in thresholds:
        output_folder = f"{base_output_folder}_{thresh}"
        ...
        for _, row in video_mapping.iterrows():
            ...
            df_distances = process_video(keypoints_file, video_id, conv_factor, thresh)
            output_file = os.path.join(output_folder, f"{video_id}.csv")
            df_distances.to_csv(output_file, index=False)
            print(f"Saved bone distances for video {video_id} with threshold {thresh} to {output_file}")
    
    print("complete for all thresholds.")
